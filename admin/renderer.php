...
class core_admin_renderer extends plugin_renderer_base {
    ...
    protected function campaign_content(bool $showcampaigncontent): string {
        ...
        $cache = cache::make('core', 'campaigncontent');
        $campaigncontenthtml = $cache->get($lang);

        if ($campaigncontenthtml) {
            $content = $campaigncontenthtml;
        } else {

            $url = "https://banners.moodle.org/donations/lms/$lang/";
            $curl = new curl();
        ...
        return $this->warning(format_text($content, FORMAT_HTML), 'secondary');
    }
