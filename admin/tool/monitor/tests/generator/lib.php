...
class tool_monitor_generator extends testing_module_generator {
     ...
     *
     * @throws coding_exception if ruleid or userid not passed
     * @param \stdClass $record
    ...
    public function create_subscription($record = null) {
        ...
        if (!isset($record->ruleid)) {
            throw new coding_exception('rule ID must be present in tool_monitor_generator::create_subscription() $record');
        }
        if (!isset($record->cmid)) {
            $record->cmid = 0;
        }
        if (!isset($record->userid)) {
            throw new coding_exception('user ID must be present in tool_monitor_generator::create_subscription() $record');
        }
