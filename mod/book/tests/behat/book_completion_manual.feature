...
Feature: Manual completion of book activity
  In order to mark a book activity as done
  As a student
  ...
  Background:
    ...
    And the following "mod_book > chapters" exist:
      ...
      | Music history | Drum theory | Rudiments are important | 1       | 0         |

  Scenario: Verify that in book activity a teacher cannot manually mark the activity as done
    Given I am on the "Music history" "book activity" page logged in as teacher1
    Then the manual completion button for "Music history" should be disabled

  @javascript
  Scenario: Verify that students can manually mark the book activity as done
