...
class mod_feedback_generator extends testing_module_generator {
     ...
     */
    protected function get_item_response_value($item, string $response) {
        if (strpos($item->typ, 'multichoice') === 0) {
            $itemclass = "feedback_item_{$item->typ}";

            return $this->get_choice_item_response_value(new $itemclass(), $item, $response);
        }
