...
Feature: Manual completion of workshop activity
  In order to mark a workshop activity as done
  As a student
  ...
  Background:
    ...
    And the following "activities" exist:
      | activity | name          | course | submissiontypefile | completion |
      | workshop | Music history | C1     | 1                  | 1          |

  Scenario: Verify that in workshop a teacher cannot manually mark the activity as done
  ...
  @javascript
  Scenario: Verify that in workshop a student can manually mark the activity as done
