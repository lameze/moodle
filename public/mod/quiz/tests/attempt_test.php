...
final class attempt_test extends \advanced_testcase {
    ...
    public function test_question_shuffle(): void {
        ...
        // Create three questions, each on a separate page.
        $questiongenerator = $this->getDataGenerator()->get_plugin_generator('core_question');
        $cat = $questiongenerator->create_question_category();
        for ($i = 0; $i < 5; $i++) {
            $question = $questiongenerator->create_question('shortanswer', null, ['category' => $cat->id]);
        ...
        // Update the first section to be shuffled and set heading.
        ...
        $attemptobj1 = \mod_quiz\quiz_attempt::create($attempt1->id);
        $attemptobj2 = \mod_quiz\quiz_attempt::create($attempt2->id);
